/*
    ...
    관계형 데이터베이스에서는 최소한의 데이터를 각각의 테이블에 답고 있음
    (무작정 다 조회해 오는게 아니라 각 테이블간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회해야 한다)
...
SELECT DEPT_ID, EMP_NAME, BONUS, DEPT_TITLE
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-------------------------------------------------------------------------
/*
    2. 포괄조인 / 외부조인 (OUTHER JOIN)
    두 테이블간에 JOIN시 일치하지 ㅇㄴㄹ하는 행도 포함시켜 조회 가능
...
--외부조인과 비교할만한 내부조인
-- 사원명, 부서명, 급여, 연봉
--부서배치를 받지 않은 2명의 사원정보 누락됨

--1) LEFT [OUTHER] JOIN : 두 테이블 중 왼편에 기술된 테이블을 기준으로 JOIN
--ANSI
...
--2) RIGHT [OTHER] JOIN : 두 테이블 중 오른편에 기술된 테이블들을 기준으로 JOIN
--ANSI
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
--FULL [OTHER] JOIN : 두 테이블이 가진 모든 행을 조회할 수 있다.
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
