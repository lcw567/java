...
--서브 쿼리 예시2)
--전 진원의 평균급여보다 더 많은 급여를 받는 사원들의 사번, 이름, 직급코드, 급여 조회
--1) 전직원의 평균 급여
SELECT ROUND(AVG(SALARY))
FROM EMPLOYEE;

--2) 3047633원보다 급여가 많은 사람들의 사번, 이름, 직급코드, 급여 조회
...
/*
    *서브쿼리의 구분
    ...
    --다중행 서브쿼리 : 서브쿼리의 조회 결곽밧이 여러행일 때(여러행 한열)
    --다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한행이지만 컬럼이 여러개일 떄
    --다둥행 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 여러행 여러 컬럼일 때
...
/*
    ...
    IN (서브쿼리) : 여러개의 결과값 주에서 한개라도 일치하는 값이 있다면 조회
    > ANY (서브쿼리) : 여러개의 결과값 중에서 한개라도 클경우
    < ANY (서브쿼리) : 여러개의 결과값 중에서 한개라도 작을경우 조회
            비교대상 > ANY (서브쿼리 결과값 -> 값1, 값2, 값3.....)
            
    > ALL서브쿼리 : 여러개의 모든 결과값들 보다 클 경우 조회
    < ALL서브쿼리 : 여러개의 모든 결과값들 보다 작을 경우 조회
            
*/

--1) 유재식 또는 윤은혜사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여조회
--1_1) 유재식 또는 윤은혜사원의 직급
SELECT JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME IN ('유재식', '윤은혜');
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME ='대리'
AND SALARY > ANY(3761234,2200000, 2500000);
...
--박나라 사원과 같은 직급코드, 같은 사수를 가지고 있는 사원들의 사번, 서원명, 직급코드, 사수번호

SELECT JOB_CODE, EMP_ID, EMP_NAME, MANAGER_ID
FROM EMPLOYEE
...
SELECT EMP_ID ,EMP_NAME , 
    ...
    CASE
        WHEN TO_NUMBER(SUBSTR(EMP_NO, 1, 2)) <= TO_NUMBER(TO_CHAR(SYSDATE, 'YY')) THEN
...
-->인라인뷰를 주로 사용하는 >> TOP-N분석 : 상위 몇개만 조회
