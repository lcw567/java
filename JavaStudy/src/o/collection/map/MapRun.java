...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		//=> 추가하고자 할때 put 메소드를 이용
		
		//1. put(k, v) : map키 벨류 세트로 추가시켜주는 메소드
		hm.put("다이제", new Snack("초코맛", 1000));
		hm.put("칸쵸", new Snack("단맛", 5000));
		hm.put("먹태깡", new Snack("짠맛", 1300));
		hm.put("아이셔", new Snack("신맛", 300));

		System.out.println(hm);
		//저장되는 순서 유지않됨! value 값이 중복되어도 키 값이 중복되지 않으면 잘 저장됨
		
		hm.put("칸쵸", new Snack("ㅋㅋㅋ", 554000));
		System.out.println(hm);
		//동일한 키값으로 다시 추가하는 경우 value값이 덮어씌워진다.!! (중복된 키 값이 공존할 수 없음 키값이 식별자 역할을 함)
		
		//2. get(Object key) : 해당 키값을 가지는 value값을 리턴해준다.
		System.out.println(hm.get("다이제"));
		
		//3.size() : ekarudlTsms rorcpemfdml tn
		...
		//5. remove(Object key) => 해당 킥밧을 찾아서 그 벨류세트를 삭제 시켜주는 메소드
		hm.remove("다이제");
		System.out.println(hm);
		...
		//1. key를 모아서 set 자료구조의 형태로 받을수 있다.
