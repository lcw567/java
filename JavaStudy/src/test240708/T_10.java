...
public class T_10 {
	...
	public static void main(String[] args) {
		/*
		 * 2차원 배열에 들어있는 데이터들 중 3의 배수만 골라내서 새로운 1차원배열에 기록하고 출력한
다. 단 중복 값은 하나만 기록되게 한다.
1. 사용 데이터
		...
		for(int i =0; i <array.length; i++) {
			for (int j=0; j< array[i].length; j++) {
				...
				if(array[i][j] %3 ==0) {//2차원 배열의 i,j의 요소가 3의 배수인지 확인
					...
					//isCountinue -> true일 때 반복값이 존재
					//isCountinue -> 반복값이 존재하지 않는다면 copyAr -> arrat[i][j]에 대입
					if(!isCountinue) {	
